...
#include<string.h>
#include<stdlib.h>
int n_of_record=0; //global variable declaration
...
void menu()
{
    ...
    printf("\n 8.Search by type");
    printf("\n 9.search all movies of any actors and actress");
    printf("\n 10.Verdict of the movies");
    printf("\n 11.Exit");
}

// for getting number of records
int get_nor()
{
    FILE *fp;
...
//for delete and update function
int avl_no(int n)
{
	FILE *fp;
	//int c=0;
...
// search all movies of any actor and actress
void search()
{
    FILE *fp2;
    char actor[20],actress[20];
    int s=0,ch2=0,ch1=0;
    do
    {
        printf("\n 1.search movies by actor name");
        printf("\n 2.search movies by actress name");
        ...
        switch(ch1)
        {
            ...
            case 2:
            {
                ...
                while (fread(&mv, sizeof(mv), 1, fp2))
                {
                    if(strcmp(actress,mv.as.name)==0)
                    {
                        printf("\n %d\t%s\t%s\t %s\t %s\t%s\t\t %d\t\t %d\t\t %d\t\t %s\t%d",mv.no,mv.name,mv.type,mv.ar.name,mv.as.name,mv.h_s.name,mv.collection,mv.budget,mv.r_d.year,mv.dr.name,mv.rating);
                        s++;
                    }
